...
public class SeedUserRoleInitial : ISeedUserRoleInitial
{
    ...
    {
        if (!await _roleManager.RoleExistsAsync("User")) //se a role não existe para o "user" cria uma role
        {
    ...
    {
        if (await _userManager.FindByEmailAsync("usuario@localhost") == null) // se o email  não existir
        {
            ...
            IdentityResult result = await _userManager.CreateAsync(user, "Numsey#2023"); // criando o usuario e senha

            if (result.Succeeded)
            {
                await _userManager.AddToRoleAsync(user, "User"); // adicionando o usuário a uma role
            }
        }
